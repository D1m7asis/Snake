...
public class Snake {
    ...
    public Snake(int x, int y) {
        ...
        GameObject fourth = new GameObject(x + 3, y);

        snakeParts.add(first);
        snakeParts.add(second);
        snakeParts.add(third);
        snakeParts.add(fourth);
    }


    public int getTailX() {
        if (direction == Direction.LEFT) {
            return snakeParts.get(snakeParts.size()-1).x + 1;
        }
        if (direction == Direction.RIGHT) {
            return snakeParts.get(snakeParts.size()-1).x - 1;
        }

        return snakeParts.get(snakeParts.size()-1).x;
    }

    public int getTailY() {
        if (direction == Direction.DOWN) {
            return snakeParts.get(snakeParts.size()-1).y - 1;
        }
        if (direction == Direction.UP) {
            return snakeParts.get(snakeParts.size()-1).y + 1;
        }

        return snakeParts.get(snakeParts.size()-1).y;
    }


    public void draw(Game game) {
        Color color = isAlive ? Color.BLACK : Color.DARKRED;
    ...
    public void move(Clover clover) {
        GameObject newHead = createNewHead();
        /* if (newHead.x > SnakeGame.WIDTH
                || newHead.x < 0
                || newHead.y > SnakeGame.HEIGHT
        ...
        if (newHead.x == clover.x && newHead.y == clover.y) {
            removeTail();
            clover.isAlive = false;
        }
